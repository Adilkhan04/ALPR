...
def ocr_image(img,coordinates):
    ...
    # Convert the image to float32 and add the batch dimension
    img = tf.cast(img, tf.float32)
    img = tf.expand_dims(img, 0)

    # Perform super resolution on the image
    super_resolution = model(img)
    super_resolution = tf.cast(tf.clip_by_value(super_resolution[0], 0, 255), tf.uint8)

    gray = cv2.cvtColor(super_resolution.numpy() , cv2.COLOR_RGB2GRAY)
